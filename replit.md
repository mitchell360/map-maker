...
- **OSRM Fallback Routing:** For locations outside the Roman road network (e.g., Mesopotamia, Arabia, Sinai), the system falls back to OSRM (Open Source Routing Machine) public API for modern road routing. Fallback routes are displayed with dashed lines to indicate they are modern approximations.
...
- **Routing:** Client-side graph-based routing using itiner-e network, with OSRM API fallback for locations outside Roman road coverage
